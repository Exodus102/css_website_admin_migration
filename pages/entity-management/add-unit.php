...
try {
    // Fetch all units
    $stmtUnits = $pdo->query("SELECT id, campus_name, division_name, unit_name FROM tbl_unit ORDER BY campus_name, division_name, unit_name ASC");
    $units = $stmtUnits->fetchAll(PDO::FETCH_ASSOC);

    // Fetch all campuses for dropdowns
...
<dialog id="add-unit-dialog" class="p-6 rounded-md shadow-lg backdrop:bg-black backdrop:bg-opacity-50 w-full max-w-md">
    <form id="add-unit-form" method="POST" class="space-y-4">
        ...
        <div>
            <label for="add-campus-name" class="block text-sm font-medium text-gray-700">CAMPUS</label>
            <select id="add-campus-name" name="campus_name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                <option value="" hidden>Name of Campus</option>
...
<dialog id="edit-unit-dialog" class="p-6 rounded-md shadow-lg backdrop:bg-black backdrop:bg-opacity-50 w-full max-w-md">
    <form id="edit-unit-form" method="POST" class="space-y-4">
        ...
        <div>
            <label for="edit-campus-name" class="block text-sm font-medium text-gray-700">CAMPUS</label>
            <select id="edit-campus-name" name="campus_name" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm" required>
                <option value="">Name of Campus</option>
                <?php foreach ($campuses as $campus) : ?>
                    <option value="<?php echo htmlspecialchars($campus); ?>"><?php echo htmlspecialchars($campus); ?></option>
                <?php endforeach; ?>
            </select>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.querySelectorAll('.edit-unit-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                document.getElementById('edit-campus-name').value = btn.dataset.campus;
